/* =============================================
   CALCULADORA DE PRECIFICAÇÃO 3D (KIT)
   ============================================= */
...
const DOM = {
    ...
    piecesPerKit: document.getElementById('piecesPerKit'),
...
function attachEventListeners() {
    // Buttons
    if (DOM.calculateBtn) DOM.calculateBtn.addEventListener('click', calculateRealtime);
    ...
    // Marketplace
    if (DOM.marketplaceSelect) DOM.marketplaceSelect.addEventListener('change', () => calculateRealtime());
...
function getInputValues() {
    ...
    return {
        filaments, auxiliaries, totalAuxiliaryCost, totalFilamentCost,
        productName: DOM.productName.value.trim() || 'Kit sem nome',
        ...
        piecesPerKit: parseInt(DOM.piecesPerKit?.value) || 1,
...
function saveProduct() {
    ...
    if (!values.productName || !values.productName.trim()) {
        showToast('Preencha o nome do kit antes de salvar!', 'error');
        return;
    }
    if (values.salePrice <= 0) {
        showToast('Defina um preço de venda antes de salvar!', 'error');
        return;
    }
    const product = {
        id: Date.now(),
    ...
    localStorage.setItem('savedProducts3d_kit', JSON.stringify(savedProducts));
    showToast('Kit salvo com sucesso!', 'success');
    renderSavedProducts();
    updateHistoryBadge();
}
...
function loadSavedProducts() {
    try {
        savedProducts = JSON.parse(localStorage.getItem('savedProducts3d_kit')) || [];
    } catch(e) {
        savedProducts = [];
    }
}
